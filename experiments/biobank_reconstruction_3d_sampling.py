...
def main(_):
    ...
    # Initialize wandb
    run = wandb.init(project="enf-min-bio-scratch-3d-sampling", config=config.to_dict(), mode="online" if not config.debug else "dryrun", name=config.run_name)
    ...
    num_subsampled_points = img_shape[0] * img_shape[1]
    ...
    def extract_z_slice(coordinates, y, slice_idx, total_slices, img_shape):
        """
        Extract coordinates for a specific z-slice from a 3D coordinate grid and sample
        corresponding values from a second array.
        
        Args:
            coordinates: JAX array of shape (batch_size, num_points, 3)
            y: JAX array of shape (batch_size, num_points, C) where C can be any dimension
            slice_idx: Index of the slice to extract (0-based, must be < total_slices)
            total_slices: Total number of slices in the volume
            img_shape: Shape of the image (H, W, D, ...)
        
        Returns:
            Tuple of JAX arrays (slice_coords, slice_y) containing only data from the specified slice
        """
        if slice_idx < 0 or slice_idx >= total_slices:
            raise ValueError(f"slice_idx must be between 0 and {total_slices-1}")
        
        # Calculate the expected z-value for this slice
        if total_slices > 1:
            target_z = -1.0 + 2.0 * slice_idx / (total_slices - 1)
        else:
            target_z = 0.0  # Special case for 1 slice
        
        # Create a mask for points where z equals the target value
        epsilon = 1e-6
        z_values = coordinates[:, :, 2]
        mask = jnp.abs(z_values - target_z) < epsilon
        ...
        slice_coords = jnp.zeros((batch_size, points_per_slice, 3))
        slice_y = jnp.zeros((batch_size, points_per_slice, y.shape[2]))  # Use y's channel dimension
        
        # Process each batch separately (avoiding JAX transformations)
        for b in range(batch_size):
            # Get indices for this slice
            indices = jnp.where(mask[b])[0]
            
            # Ensure we have the expected number of points
            if indices.shape[0] != points_per_slice:
                print(f"Warning: Expected {points_per_slice} points for slice {slice_idx}, got {indices.shape[0]}")
                # If we don't have enough points, we'll use what we have
                indices = indices[:min(indices.shape[0], points_per_slice)]
                # If we need more points, we'll pad with the last point
                if indices.shape[0] < points_per_slice:
                    indices = jnp.pad(indices, (0, points_per_slice - indices.shape[0]), 
                                    mode='constant', constant_values=indices[-1])
            
            # Select coordinates and y values for this batch using the same indices
            slice_coords = slice_coords.at[b].set(coordinates[b][indices])
            slice_y = slice_y.at[b].set(y[b][indices])
        
        return slice_coords, slice_y
    ...
    for epoch in range(config.train.num_epochs_pretrain):
        ...
        for i, (img, _) in enumerate(train_dloader):
            ...
            if glob_step % config.train.log_interval == 0:
                ...
                x_slice_1, y_slice_1 = extract_z_slice(x_i, y_i, 0, 2, img_shape) # TODO: PLOT y_slice_1
                
                # print("x_slice_1 shape: ", x_slice_1.shape) # [2, 20480, 3]
                # print("y_slice_1 shape: ", y_slice_1.shape) # [2, 20480, 1]
                
                
                # print("img_shape: ", img.shape)

                # Reconstruct and plot the first image in the batch
                img_r_slice = recon_enf.apply(recon_enf_params, x_slice_1, *z)
                
                img_r_slice = img_r_slice.reshape(img[:, :, :, :1, :].shape)
                img_slice = img[:, :, :, :1, :]
                
                # print("img_r_slice shape: ", img_r_slice.shape)
                # print("img_slice shape: ", img_slice.shape)
                
                # Note: Currently passes first imge in batch and first slice in z dimension
                fig = plot_biobank_comparison(img_slice[0], img_r_slice[0])
