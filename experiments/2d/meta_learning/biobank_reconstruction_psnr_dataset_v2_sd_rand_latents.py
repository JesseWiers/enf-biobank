...
def plot_biobank_comparison(
    ...
    # Squeeze out extra dimensions to get 2D arrays
...
def main(_):
    ...
    def evaluate_test_set(enf_params, test_dloader, key):
        ...
        for i, (img, _) in enumerate(test_dloader):
            ...
            # change context vector to all ones            
            z = (z[0], jnp.zeros((config.train.batch_size, config.recon_enf.num_latents, config.recon_enf.latent_dim)), z[2])
