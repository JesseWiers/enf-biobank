...
def plot_biobank_comparison(
    ...
    # Squeeze out extra dimensions (1,H,W,1) -> (H,W)
...
def main(_):
    ...
    train_dloader, test_dloader = get_dataloaders('2d_biobank_v2', config.train.batch_size, config.dataset.num_workers, num_train=config.dataset.num_patients_train, num_test=config.dataset.num_patients_test, seed=config.seed, mosaic_augment=config.mosaic_augment)
    ...
    for epoch in range(config.train.num_epochs_train):
        ...
        for i, (img, _) in enumerate(train_dloader):
            ...
            if glob_step % config.train.log_interval == 0:
                ...
                fig = plot_biobank_comparison(img[random_idx], img_r[random_idx], poses=z[0][0])
