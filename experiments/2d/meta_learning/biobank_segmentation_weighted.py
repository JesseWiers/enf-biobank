...
def main(_):
    ...
    def segment_outer_step(coords, img, gt_mask, recon_enf_params, seg_enf_params, seg_enf_opt_state, key):        
        ...
        def CE_loss(seg_enf_params, z, coords, gt_mask):
            ...
            # Compute per-class weights to balance the loss
            # Count number of pixels per class
            class_counts = jnp.sum(one_hot_labels, axis=(0, 1, 2))
            # Add small epsilon to prevent division by zero
            class_counts = class_counts + 1e-8
            # Compute inverse frequency weights
            class_weights = 1.0 / class_counts
            # Normalize weights to sum to number of classes
            class_weights = class_weights * (4.0 / jnp.sum(class_weights))
            
            # Apply class weights to the loss
            # Compute cross-entropy loss per class
            per_class_loss = -jnp.sum(one_hot_labels * jax.nn.log_softmax(logits, axis=-1), axis=(0, 1, 2))
            # Apply class weights
            weighted_loss = jnp.sum(per_class_loss * class_weights)
