...
def get_config():
    ...
    config.dataset.latent_dataset_path = f"/projects/prjs1252/data_jesse_v2/latent_dataset_autodecoding_{config.recon_enf.num_latents}l_{config.recon_enf.latent_dim}d_{config.dataset.num_patients_train}_less_general_patients.h5"  # Where to save the latent dataset
    
    # Optimizer config
    config.optim = ml_collections.ConfigDict()
    config.optim.inner_lr = (0., 0.1, 0.) # (pose, context, window), orginally (2., 30., 0.) # NOTE: Try 1e-3 
    config.optim.num_epochs = 100  # Number of epochs to optimize latents per patient
...
def main(_):
    ...
    x = create_coordinate_grid(batch_size=config.train.batch_size, img_shape=sample_img.shape[:2])
