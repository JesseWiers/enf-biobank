...
class PonitaFixedSize(nn.Module):
    num_hidden: int # PASS CORRECT CONFIG
    num_layers: int
    scalar_num_out: int
    vec_num_out: int
    ...
    kernel_size: Union[float, str] = "global" # NOTE: GLOBAL
    ...
    def __call__(self, latent):
        ...
        # NOTE: NEW
        invariant1 = (rel_pos * self.ori_grid).sum(axis=-1, keepdims=True)  # [num_edges, num_ori, 1]
        
        # Displacement orthogonal to the orientation (take norm in 3D)
        if self.spatial_dim == 2:
            invariant2 = (rel_pos - invariant1 * self.ori_grid).sum(axis=-1, keepdims=True)  # [num_edges, num_ori, 1]
        elif self.spatial_dim == 3:
            invariant2 = jnp.linalg.norm(rel_pos - invariant1 * self.ori_grid, axis=-1, keepdims=True)  # [num_edges, num_ori, 1]
