...
def get_config():
    ...
    config.recon_enf.num_latents = 288
    ...
    config.optim.subsample_percentage = 0.5
...
def main(_):
    ...
    num_subsampled_points = int((img_shape[0] * img_shape[1] * img_shape[2]) * config.optim.subsample_percentage)
    logging.info(f"Number of subsampled points: {num_subsampled_points}")
    ...
    def evaluate_test_set(enf_params, test_dloader, key):
        ...
        for idx, (img, _) in enumerate(test_dloader):
            ...
            # TODO: Obtain z here using random pixels
            x_i, y_i = subsample_xy(key, x, y)            
            _, (z, _) = recon_inner_loop(enf_params, x_i, y_i, key)
            ...
            for z_idx in range(img.shape[1]):
                ...
                mse = jnp.mean((img_slice - img_r_slice) ** 2)
                psnr_value = 20 * jnp.log10(1.0 / jnp.sqrt(mse))
            
                # Append to patient lists
                ...
                if idx == 0:
                    ...
                    img_slice = jnp.clip(img_slice, 0, 1)
