...
def main(_):
    ...
    def recon_inner_loop(enf_params, coords, img, key):
        ...
        # Note: make it accept z coordinate
    ...
    @jax.jit
    def recon_inner_loop_eval(enf_params, coords, img, key):
        ...
        def mse_loss(z):
            ...
            for i in range(len(list(config.dataset.z_indices))):
                
                coords_slice = coords[:, (i*num_subsampled_points):((i+1)*num_subsampled_points), :]
                img_slice = img[:, (i*num_subsampled_points):((i+1)*num_subsampled_points), :]
                
                out = recon_enf.apply(enf_params, coords_slice, *z)
                
                loss = jnp.mean((out - img_slice) ** 2)
        ...
        def psnr(z):
            ...
            return jnp.mean(jnp.array(psnr_slices))

        def inner_step(z, _):
            
            # Gradients are computed over all slices
            _, grads = jax.value_and_grad(mse_loss)(z)
            
            # Gradient descent update (z is constructed from all coords)
            z = jax.tree.map(lambda z, grad, lr : z - lr * grad, z, grads, config.optim.inner_lr)
            return z, None
    ...
    def evaluate_test_set(enf_params, test_dloader, key):
        ...
        for idx, (img, _) in enumerate(test_dloader):
            ...
            mse, (z, psnr_value) = recon_inner_loop_eval(enf_params, x, y, key)
            
            # Add these lines to append the values to your lists
            psnrs_volumes.append(psnr_value)
            mses_volumes.append(mse)
        ...
        # Calculate average PSNR - convert to array first
        avg_psnr = jnp.mean(jnp.array(psnrs_volumes)) 
        avg_mse = jnp.mean(jnp.array(mses_volumes)) 
        ...
        for i, (img, _) in enumerate(train_dloader):
            ...
            for z_idx in range(img.shape[1]):
                ...
                (loss, z, psnr_value), recon_enf_params, recon_enf_opt_state, key = recon_outer_step(
                    x_slice, y_slice, recon_enf_params, recon_enf_opt_state, key)
            ...
            if glob_step % config.train.log_interval == 0:
                ...
                logging.info(f"RECON ep {epoch} / step {glob_step} || mse: {sum(epoch_loss) / len(epoch_loss)} || psnr: {sum(epoch_psnr) / len(epoch_psnr)} || test-psnr: {test_psnr} || test-mse: {test_mse}")
                wandb.log({"train-mse": sum(epoch_loss) / len(epoch_loss), "train-psnr": sum(epoch_psnr) / len(epoch_psnr), "test-psnr": test_psnr, "test-mse": test_mse, "reconstruction": fig}, step=glob_step)
                plt.close('all')
