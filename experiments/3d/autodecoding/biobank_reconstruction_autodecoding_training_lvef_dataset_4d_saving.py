...
def get_config():
    ...
    config.recon_enf.latents_per_dim = () # (T, Z, H, W)

    # Dataset config
    config.dataset = ml_collections.ConfigDict()
    config.dataset.num_workers = 0
    config.dataset.num_patients_train = -1 
    config.dataset.num_patients_test = 10
    config.dataset.z_indices = (0, 1, 3, 4, 5, 6, 7)  # Which z-slices to use
    ...
    config.train.num_epochs_train = 6
...
def main(_):
    ...
    config.dataset.latent_dataset_path = f"/projects/prjs1252/data_jesse_v2/latent_dataset_4d_autodecoding_{config.recon_enf.num_latents}latents_{config.recon_enf.latents_per_dim}latentsperdim_{config.recon_enf.latent_dim}dim_{config.dataset.num_patients_train}patients_{config.train.num_epochs_train}epochs_evensampling{config.recon_enf.even_sampling}.h5"
    ...
    if config.recon_enf.latents_per_dim == ():
        latents_per_dim = None
    else:
        latents_per_dim = config.recon_enf.latents_per_dim
    ...
    logging.info(f"Training for {config.train.num_epochs_train} epochs")
    ...
    for epoch in range(config.train.num_epochs_train):
        ...
        # Evaluate on first 2 samples of train set 
        ...
        if epoch % 20 == 0:
            ...
            if sum(psnrs_evaluation) / len(psnrs_evaluation) > best_psnr:
                ...
                checkpoint_path = f"model_checkpoints_autodecoding/{config.exp_name}/{config.run_name}/checkpoint_{glob_step}.pkl"
    ...
    with h5py.File(config.dataset.latent_dataset_path, 'w') as f:
        ...
        for i, (img, patient_id) in enumerate(train_dloader):  # We'll need patient_id from dataloader
            patient_group = patients_group.create_group(patient_id[0])  # Assuming batch_size=1 for simplicity
