...
import itertools
# Custom imports
from experiments.datasets.autodecoding.biobank_multimodal_endpoint_dataset import get_dataloaders
...
def save_checkpoint(checkpoint_dir, model_params, optimizer_state, epoch, global_step, best_psnr):
    ...
    print(f"Checkpoint saved at epoch {epoch} in {checkpoint_dir}")
...
def get_config():
    ...
    config.dataset.num_leads = 12  # MAXIMUM IS 12
    ...
    config.recon_enf.num_out = config.dataset.num_leads + 1  # Dynamic: MRI channel + num_leads
...
def main(_):
    ...
    train_dloader = get_dataloaders(
        ...
        num_leads=config.dataset.num_leads  # Add this parameter
    )
    ...
    def train_step(x_i_mri, x_i_ecg, y_i_total, z, enf_params, enf_opt_state, key):
        def loss_fn(z, enf_params):
            ...
            y_i_hat_mri = recon_enf.apply(enf_params, x_i_mri, *z)  # (B, num_points, num_leads + 1)
            y_i_hat_ecg = recon_enf.apply(enf_params, x_i_ecg, *z)  # (B, num_points, num_leads + 1)
            
            # Compute MRI loss (only channel 0)
            mri_loss = jnp.mean((y_i_hat_mri[..., 0] - y_i_total[..., 0]) ** 2)
            
            # Compute ECG loss (channels 1 to num_leads)
    ...
    # Log output kernel shape
    output_kernel = recon_enf_params['params']['W_out']['kernel']
    logging.info(f"Current output kernel shape: {output_kernel.shape}")
    ...
    for epoch in range(config.train.num_epochs_train):
        ...
        if epoch % 10 == 0:
            ...
            if current_avg_psnr > best_psnr:
                logging.info(f"New best average PSNR: {current_avg_psnr:.2f} (previous: {best_psnr:.2f})")
                best_psnr = current_avg_psnr
                
                # Create checkpoint directory if it doesn't exist
                checkpoint_dir = os.path.join("model_checkpoints_multimodal", config.run_name)
                
                # Save checkpoint
                save_checkpoint(
                    checkpoint_dir=checkpoint_dir,
                    model_params=recon_enf_params,
                    optimizer_state=recon_enf_opt_state,
                    epoch=epoch,
            ...
            wandb.log({
                ...
                "epoch": epoch,
                "output_kernel_shape": str(output_kernel.shape)  # Add this line to log to wandb
            }, step=glob_step)
