...
def get_config():
    ...
    config.recon_enf.num_latents = 625 
    ...
    config.dataset.t_indices = (0, 10, 20, 30, 40, 49)
...
def plot_biobank_comparison(
    ...
    """Plot original and reconstructed images for all z-slices at a random time point.
    
    Args:
        original: Original images with shape (Z, T, H, W, 1)
        reconstruction: Reconstructed images with shape (Z, T, H, W, 1)
    """
    # Randomly select a time point
    key = jax.random.PRNGKey(int(time.time()))  # Use current time as seed
    t_idx = jax.random.randint(key, shape=(), minval=0, maxval=original.shape[1])
    
    # Get all Z slices for the selected time point
    original_t = original[:, t_idx]  # Shape: (Z, H, W, 1)
    reconstruction_t = reconstruction[:, t_idx]  # Shape: (Z, H, W, 1)
    
    num_slices = original.shape[0]  # Number of Z slices
    fig, axes = plt.subplots(num_slices, 2, figsize=(4, 2 * num_slices))
    fig.suptitle(f'Original (left) vs Reconstruction (right) at t={t_idx}')
    ...
    for i in range(num_slices):
        # Plot original
        axes[i, 0].imshow(original_t[i, :, :, 0], cmap='gray')
        axes[i, 0].set_title(f'Original Slice {i}')

        # Plot reconstructed
        axes[i, 1].imshow(reconstruction_t[i, :, :, 0], cmap='gray')
...
def main(_):
    ...
    img_shape = sample_img.shape[1:] # [Z, T, H, W, 1]
    ...
    for epoch in range(config.train.num_epochs_train):
        ...
        for i, img in enumerate(train_dloader):
        ...
        train_dloader_subset = itertools.islice(train_dloader, 50)
        psnrs_evaluation = []
        
        # time evaluation
        start_time = time.time()
        
        for i, img in enumerate(train_dloader_subset):
            z = jax.tree_map(
                lambda x: x[i*config.train.batch_size:(i+1)*config.train.batch_size], 
                z_dataset
            )
