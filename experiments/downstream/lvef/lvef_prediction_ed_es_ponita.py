...
def get_config():
    ...
    config.dataset.z_indices = (5,)
    config.dataset.debug_limit = None

    # Optimizer config
    config.optim = ml_collections.ConfigDict()
    config.optim.lr_classifier = 0.001
...
def main(_):
    ...
    classifier_params = classifier.init(key, temp_z)

    # Define optimizer for the classifier model
    classifier_opt = optax.adam(learning_rate=config.optim.lr_classifier)
    classifier_opt_state = classifier_opt.init(classifier_params)
    ...
    @jax.jit
    def regressor_step(z, targets, classifier_params, classifier_opt_state, key):
        ...
        def mse_loss(params):
            ...
            preds = classifier.apply(params, (p, c, g))
        ...
        # Get gradients
        loss, grads = jax.value_and_grad(mse_loss)(classifier_params)

        # Update classifier parameters
        updates, classifier_opt_state = classifier_opt.update(grads, classifier_opt_state)
        classifier_params = optax.apply_updates(classifier_params, updates)

        return loss, classifier_params, classifier_opt_state
    ...
    for epoch in range(config.train.num_epochs_train_cls):
        ...
        for batch in tqdm(train_dloader, desc=f"Epoch {epoch}"):
            ...
            # Training step
            mse_loss, classifier_params, classifier_opt_state = regressor_step(
                z, targets, classifier_params, classifier_opt_state, key
            )
        ...
        for batch in tqdm(val_dloader, desc=f"Testing"):
            ...
            # Forward pass through classifier
            preds = classifier.apply(classifier_params, (p, c, g))
