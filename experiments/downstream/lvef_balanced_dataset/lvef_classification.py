...
def get_config():
    ...
    config.dataset.t_indices = (0, 10, 20, 30, 40)
...
def main(_):
    ...
    train_dloader, val_dloader = create_dataloaders(
        ...
        t_indices=list(config.dataset.t_indices),
    ...
    temp_z = initialize_latents(
        ...
        num_latents=num_latents_slice*len(config.dataset.t_indices)*len(config.dataset.z_indices), 
    ...
    transformer_opt = optax.adam(learning_rate=config.optim.lr_model)
    ...
    @jax.jit
    def classifier_step(z, targets, transformer_params, transformer_opt_state, key):
        def cross_entropy_loss(params):
            ...
            # Convert targets to binary labels (0 if < 60, 1 if >= 60)
            binary_targets = (targets >= 60.0).astype(jnp.int32)
            
            # Forward pass through transformer
            logits = transformer.apply(params, p, c, g)
    ...
    for epoch in range(config.train.num_epochs_train_cls):
        ...
        for batch in tqdm(train_dloader, desc=f"Epoch {epoch}"):
            ...
            # Training step
            loss, transformer_params, transformer_opt_state = classifier_step(
            ...
            p, c, g = z
            c = (c - c_mean) / c_std
            logits = transformer.apply(transformer_params, p, c, g)
        ...
        logging.info(f"Epoch {epoch}: Loss = {avg_loss:.4f}, Accuracy = {avg_accuracy:.4f}")
        wandb.log({
            "train/loss": avg_loss,
