...
# Custom imports
from experiments.datasets.biobank_latent_dataset import LatentEndpointDatasetEDESMiddleSlices, collate_fn
...
def create_dataloaders(hdf5_path, endpoints_csv_path, batch_size=16, endpoint_name='LVEF', 
    ...
    """
    try:
        full_dataset = LatentEndpointDatasetEDESMiddleSlices(
            hdf5_path, 
            endpoints_csv_path, 
            endpoint_name,
...
def get_config():
    ...
    config.dataset.latent_path = '/projects/prjs1252/data_jesse_v2/latent_dataset_lvef_16l_16d_psnr_22.h5'
    ...
    config.model.name = "egnn"  # Can be "TransformerClassifier", "ponita", or "egnn"
    ...
    for epoch in range(config.train.num_epochs_train_cls):
        ...
        for batch in tqdm(train_dloader, desc=f"Training Epoch {epoch}"):
            ...
            if config.dataset.reset_es_timepoint:
                ...
                p = p.at[:, num_latents_slice:, 0].set(1)
