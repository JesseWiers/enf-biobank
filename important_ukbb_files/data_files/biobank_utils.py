...
class BaseImage(object):
    ...
    # def WriteToNifti(self, filename, use_lps=True):
        
    #     print("Writing to Nifti file using LPS: ", use_lps)
        
    #     nim = nib.Nifti1Image(self.volume, self.affine)
    #     nim.header['pixdim'][4] = self.dt
        
    #     if use_lps:
    #         # Set qform_code to indicate non-standard coordinate system
    #         nim.header['qform_code'] = 1
    #         # You might need to set additional flags to indicate LPS system
    #     else:
...
class Biobank_Dataset(object):
    ...
    def __init__(self, input_dir, cvi42_dir=None):
        ...
        # Look for all the expected subdirectories
        for s in subdirs:
        ...
        # Deal with sax directory not found
        if not sax_dir:
            ...
            if sax_mix_dir:
                ...
                d = dicom.dcmread(os.path.join(sax_mix_dir, list[0])) # CHANGED FOR NEWER DICOM VERSION
        ...
        # Deal with lax directory not found
        if not lax_2ch_dir and not lax_3ch_dir and not lax_4ch_dir:
            ...
            if lax_mix_dir:
                ...
                d = dicom.dcmread(os.path.join(lax_mix_dir, list[0])) # CHANGED FOR NEWER DICOM VERSION
                T = d.CardiacNumberOfImages
                if len(list) != 3 * T:
        ...
        # Create a dictionary of subdirectories
        self.subdir = {}
        if sax_dir:
    ...
    def find_series(self, dir_name, T):
        ...
        if len(files) > T:
            ...
            for f in files:
                ...
                d = dicom.dcmread(os.path.join(dir_name, f)) # CHANGED FOR NEWER DICOM VERSION
    ...
    def read_dicom_images(self):
        ...
        for name, dir in sorted(self.subdir.items()):
            ...
            # Read the dicom file at the first slice
            # print("os.path.join(dir[0], sorted(os.listdir(dir[0]))[0]): ", os.path.join(dir[0], sorted(os.listdir(dir[0]))[0]))
            d = dicom.dcmread(os.path.join(dir[0], sorted(os.listdir(dir[0]))[0])) # CHANGED FOR NEWER DICOM VERSION
            ...
            # Read a dicom file from the correct series when there are multiple time sequences (doesn't appear to do much in our case)
            # print("os.path.join(dir[0], self.find_series(dir[0], T)[0]): ", os.path.join(dir[0], self.find_series(dir[0], T)[0]))
            d = dicom.dcmread(os.path.join(dir[0], self.find_series(dir[0], T)[0])) # CHANGED FOR NEWER DICOM VERSION
            # d = dicom.read_file(os.path.join(dir[0], self.find_series(dir[0], T)[0]))

            # Number of columns (seems to be image height)
            X = d.Columns
            print("Number of columns (X): ", X)
            # Number of rows (seems to be image width)
            Y = d.Rows
            print("Number of rows (Y): ", Y)
            # Number of slices (seems to be image width)
            ...
            # Pixel spacing in the x direction (physical distance between pixels in the x direction in mm)
            dx = float(d.PixelSpacing[1])
            print("Pixel spacing in the x direction (dx): ", dx)
            # Pixel spacing in the y direction (physical distance between pixels in the y direction in mm)
            dy = float(d.PixelSpacing[0])
            print("Pixel spacing in the y direction (dy): ", dy)
            
            
            # TODO: might have to look into this one more time
            ...
            # print("d.ImagePositionPatient: ", d.ImagePositionPatient)
            pos_ul = np.array([float(x) for x in d.ImagePositionPatient])
             
            pos_ul[:2] = -pos_ul[:2] # REMOVED TO NOT DO THE FLIPPING !!!!

            # Image orientation
            axis_x = np.array([float(x) for x in d.ImageOrientationPatient[:3]])
            axis_y = np.array([float(x) for x in d.ImageOrientationPatient[3:]])
            
            axis_x[:2] = -axis_x[:2] # REMOVED TO NOT DO THE FLIPPING !!!!
            axis_y[:2] = -axis_y[:2] # REMOVED TO NOT DO THE FLIPPING !!!!

            if Z >= 2:
                # Read a dicom file at the second slice
                # d2 = dicom.read_file(os.path.join(dir[1], sorted(os.listdir(dir[1]))[0]))
                d2 = dicom.dcmread(os.path.join(dir[1], sorted(os.listdir(dir[1]))[0])) # CHANGED FOR NEWER DICOM VERSION
                pos_ul2 = np.array([float(x) for x in d2.ImagePositionPatient])
                pos_ul2[:2] = -pos_ul2[:2] # REMOVED TO NOT DO THE FLIPPING !!!!
            ...
            for z in range(0, Z):
                ...
                for f in files:
                    ...
                    d = dicom.dcmread(os.path.join(dir[z], f)) # CHANGED FOR NEWER DICOM VERSION
                    t = d.TriggerTime
                    files_time += [[f, t]]
                ...
                for t in range(0, T):
                    ...
                    try:
                        ...
                        # d = dicom.read_file(os.path.join(dir[z], f))
                        d = dicom.dcmread(os.path.join(dir[z], f)) # CHANGED FOR NEWER DICOM VERSION
                        volume[:, :, z, t] = d.pixel_array.transpose() # REMOVED TO WORK WITH LPS !!!!
            ...
            try:
                ...
                print("dt found, dt: ", dt)
            except IndexError:
                print("dt not found, setting dt to: ", dt)
                dt = 1
    ...
    def get_metadata(self):
        """
        Collect metadata for all image series.
        
        Returns:
        --------
        list: List of dictionaries containing metadata for each series
        """
        metadata = []
        
        # Collect metadata for each image series
        for name, image in self.data.items():
            # Skip label maps for metadata export
            if name.startswith('label_'):
                continue
                
            # Get volume shape
            if len(image.volume.shape) == 4:
                X, Y, Z, T = image.volume.shape
            else:
                X, Y, Z = image.volume.shape
                T = 1
                
            # Extract spacing information
            # dx, dy from the affine matrix
            dx = np.sqrt(np.sum(image.affine[:3, 0]**2))
            dy = np.sqrt(np.sum(image.affine[:3, 1]**2))
            dz = np.sqrt(np.sum(image.affine[:3, 2]**2))
            dt = image.dt
            ...
            metadata.append({
                ...
                'dz (mm)': dz,
                'dt (s)': dt,
                'Volume (ml)': X * Y * Z * dx * dy * dz * 1e-3  # Convert mm³ to ml
            })
